...
_city_data = {
    'Lutsk': ('50:45:33.6', '25:20:32.8', 192.0, 'Europe/Kiev'),  # Ukraine
    'Kiev': ('50.45', '30.5233333', 157.210175, 'Europe/Kiev'),  # Ukraine
    ...
    'San Francisco': ('37.7749295', '-122.4194155', 15.557819),  # USA
    ...
    'Abu Dhabi': ('24.4666667', '54.3666667', 6.296038),  # Uned Arab Emirates
    ...
    'St. Petersburg': ('59.939039', '30.315785', 11.502971),  # Russia
...
def geo(name="Lutsk", pas="fallcon777"):
    g = geocoder.geonames(name, key=pas)
    print("geoname", g, g.country, g.state, "popul - ", g.population)
    g_full = geocoder.geonames(g.geonames_id, method='details', key=pas)
    pl = eph.Observer()
    zon = g_full.timeZoneId
    pl.lat, pl.long, pl.horizon, pl.elevation = g.lat, g.lng, '-0:34', g_full.srtm3
    print("lat:", pl.lat, " long:", " tzone:", zon)
    return pl, zon
...
def city(name):
    o = eph.Observer()
    o.name = name
    if name in _city_data:
        print('City is in database')
        data = _city_data[name]
        o.lat, o.lon, o.elevation, zon = data
        o.compute_pressure()
    else:
        print("Try internet geoservis")
        o, zon = geo(name)
    return o, zon
